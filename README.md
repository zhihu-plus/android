...
- 阅读
  ...
  - TalkBack 读屏优化（信息流与详情页语义；读屏开启时关闭切换手势并保留操作栏）
...
开启系统 TalkBack 后，信息流、回答、问题、想法和评论会按页面结构朗读。标题标为 heading，图标按钮使用中文动作描述，装饰性头像和缩略图不再打断阅读。

读屏开启时会关闭回答切换手势，避免和 TalkBack 滑动抢手势；顶栏、底栏和「下一个回答」按钮不再自动隐藏，也不能进入沉浸式。回答标题提供「上一个回答 / 下一个回答」自定义动作。

不需要单独开关，跟随系统读屏。老年模式仍只负责放大文字，给低视力用户使用。
