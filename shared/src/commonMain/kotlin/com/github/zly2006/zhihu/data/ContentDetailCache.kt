...
fun zhihuContentDetailInclude(destination: NavDestination): String = when (destination) {
    is Article -> when (destination.type) {
        ArticleType.Article -> "content,topics,paid_info,can_comment,excerpt,thanks_count,voteup_count,comment_count,visited_count,relationship,ip_info,relationship.vote,author.badge_v2,author.is_following"
        ArticleType.Answer -> ".settings,content,editable_content,paid_info,can_comment,excerpt,thanks_count,voteup_count,comment_count,visited_count,attachment,reaction,ip_info,pagination_info,endorsements,question.topics,question.author,reaction.relation.voting,author.badge_v2,author.is_following,settings.table_of_contents.enabled"
    }
